...
func AutoGenRouteImport(moduleName, scanRoot, outputDir, matchPatterns, ignoreDirs string) error {
	...
	//		} else if !hasInit {
	//			// 3. 未调用则自动注入
	//			if err := injectRoutesInitToMain(mainGoPath); err != nil {
	...
	// 在AutoGenRouteImport函数末尾调用
	if len(routes) > 0 {
		// 查找业务项目的main.go
		mainGoPath, err := findMainGo(scanRoot)
		if err != nil {
			fmt.Printf("[go-common] 警告：未找到main.go：%v\n", err)
			return nil
		}
		// 调用注入函数
		err = injectRoutesInitToMain(mainGoPath, moduleName, outputDir)
		if err != nil {
			fmt.Printf("[go-common] 警告：注入失败（请手动添加）：%v\n", err)
		}
	}
...
//func injectRoutesInitToMain(mainGoPath string) error {
//	// 1. 读取main.go内容
//	content, err := os.ReadFile(mainGoPath)
//	if err != nil {
