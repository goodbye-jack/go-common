...
这里要特别强调一件事：如果业务系统希望启用当前版本新增的“回调驱动读模型增强第一版”，就应该把 `callbackUrl` 指向 `go-common/workflow` 自己暴露的回调入口，而不是继续在业务系统里单独维护一套工作流事件缓存。

当前推荐值是：

- `callbackUrl = http://{业务服务地址}/flowable/callback`
...
这个接口由 `go-common/workflow` 直接注册，用来接收 Flowable 回调事件。

业务系统需要做的事情只有两件：

第一，在发起流程时把 `callbackUrl`、`callbackEvents` 作为流程变量带进去。

第二，如果配置了回调签名密钥，就确保 Flowable 发送的请求头里带上：

- `X-Timestamp`
- `X-Nonce`
- `X-Signature`

当前第一版回调能力会直接驱动以下平台读模型：
...
这意味着后续待办、已办和流程摘要会越来越优先走平台本地读模型，而不是每次查询都完全回源 Flowable。
...
但**LDAP 登录后本地用户应该落到哪个业务角色**，仍然属于业务项目自己的职责。

原因很简单：

- 这件事天然依赖业务角色体系
- 不同行业项目的角色码、岗位码、部门码都不同
- 不能把某个业务系统的角色映射硬编码进 `go-common`

推荐做法是：

1. `go-common` 只负责把 HTTP / LDAP / JWT 的身份编码归一到工作流契约
2. 业务系统自己的 `login_ldap` / SSO 登录逻辑负责把本地用户同步到正确业务角色
3. 如果希望这部分更易测试，建议业务项目把“角色决策 + 档案同步规则”单独抽成纯逻辑包
4. BPMN 固定候选组只依赖最终稳定角色编码

这样才能保证：

- `HTTP / LDAP` 切换时尽量不改 BPMN
- `go-common` 保持长期可复用

## 10. 启动期排障建议

接入后，建议首先检查启动日志中的工作流配置摘要。
...
- `workflow.*` 配置路径是否正确
- 环境覆盖配置是否生效
- Flowable 候选组编码是否与 `role_prefix/group_prefix` 对齐
- `workflow.flowable.*`
- `workflow.api.*`
- `workflow.context.*`
- `workflow.identity.*`（推荐；用于统一 LDAP / HTTP / JWT 身份编码）
- `workflow.directory.*`（可选；未配置时默认 `none`）

如果开启表单引用解析，并且 Flowable 表单元数据不在业务库里，还需要配置：

- `workflow.formref.db_instance`

如果启用回调驱动读模型增强第一版，还建议同时配置：
...
除了 `config.yaml` 之外，还必须完成下面这些运行时动作，否则配置写对了也接不起来：

- 业务系统登录态必须能解析出当前 `userId`
- 业务系统必须能给请求上下文注入 `tenantId`
- 如果登录态里的用户标识不是 Flowable 直接使用的用户 ID，需要配置 `workflow.context.user_id_strategy`
- 如果业务侧不使用默认请求头，还要同步改 `workflow.context.*` 头名配置
- 启动阶段必须执行 `workflowapi.MustRegisterFromConfig(server)` 或 `workflowapi.RegisterFromConfig(server)`
- 业务系统必须把自己的发起接口封装好，再由它内部调用 `POST /api/process/start`
- 如果启用了回调能力，业务服务必须对外暴露 `POST /flowable/callback`

启动成功后，日志里至少会打印下面这些摘要，开发者可以直接据此排查配置问题：
...
关于完整配置模板、字段解释、Flowable 平台侧需要怎么配，请直接看《工作流配置说明》。
...
- `workflow.api.*` 是工作流标准路由、登录态和回调入口配置
- `workflow.context.*` 是工作流读取当前用户扩展头的配置
- `workflow.formref.db_instance` 只在启用表单引用解析时需要
- LDAP 配置用于当前人、上级、部门能力

如果你需要一份可以直接复制到业务项目里的完整说明，而不是这里只看示例，请直接使用《工作流配置说明》。
